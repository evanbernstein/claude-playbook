...
- The plan document itself must be sequenced red/green: for each behavioral change, list the failing test before the implementation that makes it pass. A test list dumped at the bottom of the plan is not a TDD plan; the reader should see, change by change, what fails before what's written.
