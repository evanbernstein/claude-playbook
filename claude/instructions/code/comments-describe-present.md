...
Code comments describe **what the code is and does today**, not what a future phase will do, not how today's code differs from a previous or counterfactual version, and not what fix this code represents. All three variants go stale immediately, mean nothing to a reader who isn't in the current planning conversation, and create invisible coupling between code and ephemeral artifacts.
...
- Explaining *what bug this fixes* = bad. The comment should read as if the code had always been this way. Especially common in test comments written right after a fix.
- TDD gates / `test.skip` blocks are an exception **if** the comment is pinned to a ticket/issue link, not a phase name. "Pending `meta.width` implementation, see #123" is fine.
...
**Anti-patterns to refuse, especially in test comments written right after a fix:**
...
**Self-check before writing any comment:** if I delete the bug context, does the comment still make sense as a description of what the code does? If not, rewrite it. Apply this at write time, not review time.
