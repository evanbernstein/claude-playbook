...
List every file touched (`git status`, `git diff --stat`). Open each one and check:
...
### 2. Refactor anything that stands out
Don't defer cleanup. If something feels off on the re-read, fix it in the same unit of work.

### 3. Re-run validations
Run the project's typecheck, test, and lint commands. Format only the files you touched. The cleanup itself can introduce regressions; verify.

### 4. Present the work with a clear summary
End the turn with:
