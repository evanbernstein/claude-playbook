...
**Why:** Both framings impose a negative or deviation lens on intentional decisions. "Plays column showed filter-scoped counts under filters" wasn't a bug; it was a previous design choice, and the current change just changes the behavior. A chart-view implementation isn't a "pivot" from a planned table-view; the user considered it done as intended, not a deviation.
