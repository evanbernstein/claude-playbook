...
- **Prefer the Grep/Glob tools over Bash for discovery.** To find files or search code, use the Grep and Glob tools, not `grep` / `find` / `ls` through Bash. They bypass the shell so they never trip the allowlist or prompt, and even a lone `grep`/`find` prompts more than you'd think (globs, redirects, pipes all push the command off any allowed prefix). Only drop to Bash for what the tools can't do (git, make, scripts). See [claude/instructions/tools-and-shell/prefer-search-tools-over-bash.md](claude/instructions/tools-and-shell/prefer-search-tools-over-bash.md).
